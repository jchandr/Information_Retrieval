...
public class assignmentP3 {
	...
	public static void return_scores_for_query(ArrayList<String> query) throws Exception
	{
		...
		ArrayList<Double> reference = new ArrayList<Double>(scores);
		sorted_indexes = new int[dictionary.size()];
		Collections.sort(reference);
		for(int n=0; n < docno.size();n++)
			sorted_indexes[n] = scores.indexOf(reference.get(n));
	}
